...
fn insert_into_hash_map(
    dict: &mut HashMap<Vec<String>, Vec<String>>,
    prior_ngram: Vec<String>,
    current: String,
) {
    match dict.entry(prior_ngram) {
        Entry::Vacant(e) => {
            e.insert(vec![current]);
        }
        Entry::Occupied(mut e) => {
            e.get_mut().push(current);
        }
    }
}
...
fn get_random_n_gram(dict: &HashMap<Vec<String>, Vec<String>>) -> Option<Vec<String>> {
    ...
    let binding = dict.keys().cloned().collect::<Vec<Vec<String>>>();
