...
function TextInput({ value, setValue }:
  ...
  const endEditing = useCallback(() => {
    ...
    if (input)
      input.removeEventListener('focusout', endEditing);
    setEditing(false);
    if (initialValue) {
    ...
    <div className="flex flex-row m-2">
      {
        editing ?
          <>
            <input onKeyDown={handleEnter} ref={editBox} value={value} onChange={e => setValue(e.target.value)}></input>
            <div className="cursor-pointer ml-2 flex flex-col justify-center items-center"><MdCheckCircleOutline /></div>
          </>
          :
          <>
